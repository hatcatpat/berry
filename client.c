...
static const struct command command_table[] = {
    ...
    {"switch_workspace", IPCSwitchWorkspaceRelative, false, 1, fn_int},
    {"switch_workspace_absolute", IPCSwitchWorkspace, false, 1, fn_int},
    {"send_to_workspace_absolute", IPCSendWorkspace, false, 1, fn_int},
    {"send_to_workspace", IPCSendWorkspaceRelative, false, 1, fn_int},
    ...
    {"move_step", IPCMoveStep, true, 1, fn_int},
    {"resize_step", IPCResizeStep, true, 1, fn_int}};

static void fn_hex(long *data, bool b, int i, char **argv) {
  UNUSED(b);
  data[i + b] = strtoul(argv[i - 1], NULL, 16);
}

static void fn_int(long *data, bool b, int i, char **argv) {
  UNUSED(b);
  data[i + b] = strtol(argv[i - 1], NULL, 10);
}

static void fn_bool(long *data, bool b, int i, char **argv) {
  UNUSED(b);
  data[i + b] = strcmp(argv[i - 1], "true") == 0 ? 1 : 0;
}

static void fn_str(long *data, bool b, int i, char **argv) {
  UNUSED(b);
  // lord forgive me for I have sinned
  if (strcmp(argv[i - 1], "Dialog") == 0)
    data[i + b] = Dialog;
  else if (strcmp(argv[i - 1], "Toolbar") == 0)
    data[i + b] = Toolbar;
  else if (strcmp(argv[i - 1], "Menu") == 0)
    data[i + b] = Menu;
  else if (strcmp(argv[i - 1], "Splash") == 0)
    data[i + b] = Splash;
  else if (strcmp(argv[i - 1], "Utility") == 0)
    data[i + b] = Utility;
  else if (strcmp(argv[i - 1], "Mod1") == 0)
    data[i + b] = Mod1Mask;
  else if (strcmp(argv[i - 1], "Mod2") == 0)
    data[i + b] = Mod2Mask;
  else if (strcmp(argv[i - 1], "Mod3") == 0)
    data[i + b] = Mod3Mask;
  else if (strcmp(argv[i - 1], "Mod4") == 0)
    data[i + b] = Mod4Mask;
}
 ...
 */
static void fn_font(long *data, bool b, int i, char **argv) {
  UNUSED(b);
  UNUSED(i);
  UNUSED(data);
  char **font_list;
  ...
  font_list[0] = argv[0];
  Xutf8TextListToTextProperty(display, font_list, 1, XUTF8StringStyle,
                              &font_prop);
  XSetTextProperty(display, root, &font_prop,
 ...
 */
static void fn_int_str(long *data, bool b, int i, char **argv) {
  UNUSED(b);
  UNUSED(data);
  UNUSED(i);
  char *name;
  ...
  name = argv[1];
  idx = atoi(argv[0]);
  XGetTextProperty(display, root, &text_prop,
...
static void send_command(const struct command *c, int argc, char **argv) {
  XEvent ev;
  UNUSED(argc);
   ...
   * IPCConfig at d[0]. Then, assign the specific config element at d[1],
   * shifting all values up by one. Otherwise, set the IPC command at d[0] and
   * assign arguments from 1 upwards.
   */
  if (c->config) {
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < (int)(sizeof command_table / sizeof command_table[0]);
       i++) {

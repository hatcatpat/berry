...
static struct client
    *c_list[WORKSPACE_NUMBER]; /* 'stack' of managed clients in drawing order */
static struct client
    *f_list[WORKSPACE_NUMBER]; /* ordered lists for clients to be focused */
static struct monitor *m_list = NULL;   /* All saved monitors */
static struct config conf;              /* gloabl config */
static int ws_m_list[WORKSPACE_NUMBER]; /* Mapping from workspaces to associated
                                           monitors */
...
static void client_center_in_rect(struct client *c, int x, int y, unsigned w,
                                  unsigned h);
...
static void client_fullscreen(struct client *c, bool toggle, bool fullscreen,
                              bool max);
...
static void client_set_color(struct client *c, unsigned long i_color,
...
static void ipc_switch_ws_relative(long *d);
static void ipc_send_to_ws(long *d);
static void ipc_send_to_ws_relative(long *d);
...
static void ipc_move_step(long *d);
static void ipc_resize_step(long *d);
...
static const ipc_event_handler_t ipc_handler[IPCLast] = {
    ...
    [IPCSwitchWorkspaceRelative] = ipc_switch_ws_relative,
    [IPCSendWorkspace] = ipc_send_to_ws,
    [IPCSendWorkspaceRelative] = ipc_send_to_ws_relative,
    ...
    [IPCMoveStep] = ipc_move_step,
    [IPCResizeStep] = ipc_resize_step,
    [IPCConfig] = ipc_config};

static unsigned euclidean_distance(const struct client *a,
                                   const struct client *b) {
...
/* Move a client to the center of the screen, centered vertically and
 * horizontally by the middle of the Client
 */
static void client_center(struct client *c) {
  int mon;
  LOGN("Centering Client");
  mon = ws_m_list[c->ws];
  client_center_in_rect(c, m_list[mon].x, m_list[mon].y, m_list[mon].width,
                        m_list[mon].height);
}

static int ceil10(int n) { return (n + 9) - (n + 9) % 10; }

static void client_center_in_rect(struct client *c, int x, int y, unsigned w,
                                  unsigned h) {
  LOGP("Centering at x=%d, y=%d, w=%d, h=%d", x, y, w, h);
  int new_x = ceil10(x + (conf.left_gap - conf.right_gap) / 2 + w / 2 -
                     c->geom.width / 2);
  int new_y = ceil10(y + (conf.top_gap - conf.bot_gap) / 2 + h / 2 -
                     c->geom.height / 2);
...
static void draw_text(struct client *c, bool focused) {
  ...
  XftTextExtentsUtf8(display, font, (XftChar8 *)c->title, strlen(c->title),
                     &extents);
  ...
  draw = XftDrawCreate(display, c->dec, DefaultVisual(display, screen),
                       DefaultColormap(display, screen));
  xft_render_color = focused ? &xft_focus_color : &xft_unfocus_color;
  XftDrawStringUtf8(draw, xft_render_color, font, x, y, (XftChar8 *)c->title,
                    strlen(c->title));
  XftDrawDestroy(draw);
}
...
static void client_decorations_create(struct client *c) {
  ...
  XGrabButton(display, 1, AnyModifier, c->dec, True,
...
/* Set the given Client to be fullscreen. Moves the window to fill the
 * dimensions of the given display. Updates the value of
 * _NET_WM_STATE_FULLSCREEN to reflect fullscreen changes
 */
static void client_fullscreen(struct client *c, bool toggle, bool fullscreen,
                              bool max) {
  int mon;
...
/* Focus the next window in the list. Windows are sorted by the order in which
 * they are created (mapped to the window manager)
 */
...
static void focus_best(struct client *c) {
  if (c == NULL)
    return;
...
static void handle_button_press(XEvent *e) {
  ...
  if (XGrabPointer(display, root, False, MOUSEMASK, GrabModeAsync,
                   GrabModeAsync, None, move_cursor,
  ...
  do {
    ...
    case MotionNotify:
      ...
      } else if (ev.xbutton.state ==
                 (unsigned)(conf.resize_mask | Button1Mask)) {
        nw = ev.xmotion.x - x;
        nh = ev.xmotion.y - y;
        if (conf.edge_lock)
          client_resize_relative(c, nw - c->geom.width + ocw,
                                 nh - c->geom.height + och);
        else
...
static void handle_configure_request(XEvent *e) {
  ...
  if (c != NULL) {
    ...
    client_move_relative(c, wc.x - get_actual_x(c) - 2 * left_width(c),
                         wc.y - get_actual_y(c) - 2 * top_height(c));
    client_resize_relative(
        c, wc.width - get_actual_width(c) + 2 * get_dec_width(c),
...
static void handle_unmap_notify(XEvent *e) {
  ...
  if (c != NULL) {
    ...
    /* Some applications *ahem* Spotify *ahem*, don't seem to place nicely with
     * being deleted. They close slowing, causing focusing issues with unmap
     * requests. Check to see if the current workspace is empty and, if so,
     * focus the root client so that we can pick up new key presses..
     */
    if (f_list[curr_ws] == NULL) {
      LOGN("Client not found while deleting and ws is empty, focusing root "
           "window");
      client_manage_focus(NULL);

    } else {
      LOGN(
          "Client not found while deleting and ws is non-empty, doing nothing");
    }
  }
}
...
static void ipc_switch_ws_relative(long *d) {
  int v = d[1];
  int ws =
      ((curr_ws + v) % WORKSPACE_NUMBER + WORKSPACE_NUMBER) % WORKSPACE_NUMBER;
  switch_ws(ws);
}
...
static void ipc_move_step(long *d) {
  int step = d[1];
  conf.m_step = step <= 0 ? 1 : step;
}

static void ipc_resize_step(long *d) {
  int step = d[1];
  conf.r_step = step <= 0 ? 1 : step;
}
...
static void load_color(XftColor *dest_color, unsigned long raw_color) {
  ...
  XftColorFree(display, DefaultVisual(display, screen),
               DefaultColormap(display, screen), dest_color);
  XftColorAllocValue(display, DefaultVisual(display, screen),
                     DefaultColormap(display, screen), &r_color, dest_color);
}
...
static void manage_new_window(Window w, XWindowAttributes *wa) {
  ...
  int di;
  unsigned long dl;
  if (XGetWindowProperty(display, w, net_atom[NetWMWindowType], 0, sizeof(Atom),
    ...
    if (prop_ret) {
      ...
      if ((prop == net_atom[NetWMWindowTypeDock] && !conf.manage[Dock]) ||
        ...
        LOGN("Window is of type dock, toolbar, utility, menu, or splash: not "
             "managing");
  ...
  c->geom.height =
      wa->height + 2 * (conf.b_width + conf.i_width) + conf.t_height;
  c->hidden = false;
  c->fullscreen = false;
  c->mono = false;
  c->was_fs = false;
  ...
  XGrabButton(display, 1, conf.move_mask, c->window, True,
              ButtonPressMask | ButtonReleaseMask | PointerMotionMask,
              GrabModeAsync, GrabModeAsync, None, None);
  XGrabButton(display, 1, conf.resize_mask, c->window, True,
              ButtonPressMask | ButtonReleaseMask | PointerMotionMask,
              GrabModeAsync, GrabModeAsync, None, None);
  client_manage_focus(c);
}
  ...
  for (int i = 0; i < WORKSPACE_NUMBER; i++)
    for (struct client *tmp = c_list[i]; tmp != NULL; tmp = tmp->next) {
      XGrabButton(display, 1, conf.move_mask, tmp->window, True,
                  ButtonPressMask | ButtonReleaseMask | PointerMotionMask,
                  GrabModeAsync, GrabModeAsync, None, None);
      XGrabButton(display, 1, conf.resize_mask, tmp->window, True,
                  ButtonPressMask | ButtonReleaseMask | PointerMotionMask,
                  GrabModeAsync, GrabModeAsync, None, None);
    }
}
...
static void client_move_absolute(struct client *c, int x, int y) {
  x = nearest(x, conf.m_step);
  y = nearest(y, conf.m_step);
  int dest_x = x;
  int dest_y = y;
...
static void client_move_relative(struct client *c, int x, int y) {
  ...
  if (conf.edge_lock) {
    ...
    if (c->geom.x + c->geom.width + x >
        m_list[mon].width + m_list[mon].x - conf.right_gap)
    ...
    if (c->geom.y + c->geom.height + y >
        m_list[mon].height + m_list[mon].y - conf.bot_gap)
...
static void client_monocle(struct client *c) {
  ...
  if (c->mono) {
    ...
    c->prev.width = c->geom.width;
    c->prev.height = c->geom.height;
    client_move_absolute(c, m_list[mon].x + conf.left_gap,
                         m_list[mon].y + conf.top_gap);
    client_resize_absolute(c,
                           m_list[mon].width - conf.right_gap - conf.left_gap,
                           m_list[mon].height - conf.top_gap - conf.bot_gap);
    c->mono = true;
  }
}

static void client_place(struct client *c) {
  int width, height, mon, count, max_height, t_gap, b_gap, l_gap, r_gap, x_off,
      y_off;
  ...
  for (struct client *tmp = f_list[curr_ws]; tmp != NULL; tmp = tmp->next) {
    if (tmp != c) {
      ...
      for (int i = geom->y / PLACE_RES;
           i < (geom->y / PLACE_RES) + (geom->height / PLACE_RES) &&
           i < height + y_off;
           i++) {
        for (int j = geom->x / PLACE_RES;
             j < (geom->x / PLACE_RES) + (geom->width / PLACE_RES) &&
             j < width + x_off;
             j++) {
  ...
  for (int i = height - b_gap - 1; i >= c->geom.height / PLACE_RES; i--) {
    for (int j = l_gap; j < width - r_gap; j++) {
      ...
      if (count >= c->geom.width / PLACE_RES) {
        int place_x =
            x_off * PLACE_RES +
            MAX(conf.left_gap, ceil10((j - count) * PLACE_RES +
                                      (count * PLACE_RES - c->geom.width) / 2));
        int place_y =
            y_off * PLACE_RES +
            MAX(conf.top_gap,
                ceil10((i - max_height + 1) * PLACE_RES +
                       (max_height * PLACE_RES - c->geom.height) / 2));
...
static void monitors_setup(void) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    LOGP("Screen #%d with dim: x=%d y=%d w=%d h=%d", m_list[i].screen,
         m_list[i].x, m_list[i].y, m_list[i].width, m_list[i].height);
  }
...
static void client_resize_absolute(struct client *c, int w, int h) {
  ...
  if (c->decorated) {
    ...
    XResizeWindow(display, c->dec, MAX(dec_w, MINIMUM_DIM),
                  MAX(dec_h, MINIMUM_DIM));
  }
...
static void client_resize_relative(struct client *c, int w, int h) {
  if (conf.edge_lock) {
     ...
     */
    if (c->geom.x + c->geom.width + w >
        m_list[mon].x + m_list[mon].width - conf.right_gap)
     ...
     * amount to move only to the edge of the monitor.
     */
    if (c->geom.y + c->geom.height + conf.t_height + h >
        m_list[mon].y + m_list[mon].height - conf.bot_gap)
...
/* This method will return true if it is safe to show a client on the given
 * workspace based on the currently focused workspaces on each monitor.
 */
static bool safe_to_focus(int ws) {
  ...
  for (int i = 0; i < WORKSPACE_NUMBER; i++)
    if (i != ws && ws_m_list[i] == mon && c_list[i] != NULL &&
        c_list[i]->hidden == false)
      return false;
...
static void client_send_to_ws(struct client *c, int ws) {
  ...
  client_move_absolute(c, m_list[mon_next].x + x_off,
                       m_list[mon_next].y + y_off);
...
static void client_set_title(struct client *c) {
  ...
  if (tp.encoding == XA_STRING) {
    ...
    if (XmbTextPropertyToTextList(display, &tp, &slist, &count) >= Success &&
        count > 0 && *slist) {
...
static void setup(void) {
  ...
  display_height = DisplayHeight(
      display,
      screen); /* Display height/width still needed for hiding clients */
  ...
  XSelectInput(display, root,
               StructureNotifyMask | SubstructureRedirectMask |
                   SubstructureNotifyMask | ButtonPressMask | Button1Mask);
  ...
  net_atom[NetWMStateFullscreen] =
      XInternAtom(display, "_NET_WM_STATE_FULLSCREEN", False);
  net_atom[NetWMMoveResize] =
  ...
  net_berry[BerryWindowStatus] =
      XInternAtom(display, "BERRY_WINDOW_STATUS", False);
  net_berry[BerryClientEvent] =
      XInternAtom(display, "BERRY_CLIENT_EVENT", False);
  net_berry[BerryFontProperty] =
      XInternAtom(display, "BERRY_FONT_PROPERTY", False);
  ...
  XChangeProperty(display, check, net_atom[NetWMName], utf8string, 8,
                  PropModeReplace, (unsigned char *)__WINDOW_MANAGER_NAME__, 5);
  ...
  XftColorAllocName(display, DefaultVisual(display, screen),
                    DefaultColormap(display, screen), TEXT_FOCUS_COLOR,
                    &xft_focus_color);
  XftColorAllocName(display, DefaultVisual(display, screen),
                    DefaultColormap(display, screen), TEXT_UNFOCUS_COLOR,
                    &xft_unfocus_color);
...
static void client_snap_left(struct client *c) {
  ...
  client_move_absolute(c, m_list[mon].x + conf.left_gap,
                       m_list[mon].y + conf.top_gap);
  client_resize_absolute(c, m_list[mon].width / 2 - conf.left_gap,
                         m_list[mon].height - conf.top_gap - conf.bot_gap);
}
...
static void client_snap_right(struct client *c) {
  int mon;
  mon = ws_m_list[c->ws];
  client_move_absolute(c, m_list[mon].x + m_list[mon].width / 2,
                       m_list[mon].y + conf.top_gap);
  client_resize_absolute(c, m_list[mon].width / 2 - conf.right_gap,
                         m_list[mon].height - conf.top_gap - conf.bot_gap);
}
...
static void warp_pointer(struct client *c) {
  XWarpPointer(display, None, c->dec, 0, 0, 0, 0, c->geom.width / 2,
               c->geom.height / 2);
}
...
static void client_set_status(struct client *c) {
  ...
  if (conf.json_status) {
    size = asprintf(&str,
                    ...
                    "\"decorated\":\"%s\""
                    "}",
                    (unsigned int)c->window, c->geom.x, c->geom.y,
                    c->geom.width, c->geom.height, state, decorated);
  } else {
    size = asprintf(&str,
                    "0x%08x, " // window id
                    "%d, "     // x
  ...
  XChangeProperty(display, c->window, net_berry[BerryWindowStatus], utf8string,
...
static void ewmh_set_fullscreen(struct client *c, bool fullscreen) {
  XChangeProperty(
      display, c->window, net_atom[NetWMState], XA_ATOM, 32, PropModeReplace,
      (unsigned char *)&net_atom[NetWMStateFullscreen], fullscreen ? 1 : 0);
}
...
static void ewmh_set_desktop(struct client *c, int ws) {
  ...
  data[0] = ws;
  XChangeProperty(display, c->window, net_atom[NetWMDesktop], XA_CARDINAL, 32,
                  PropModeReplace, (unsigned char *)data, 1);
}
...
static void ewmh_set_frame_extents(struct client *c) {
  ...
  data[2] = top;
  data[3] = bottom;
  XChangeProperty(display, c->window, net_atom[NetWMFrameExtents], XA_CARDINAL,
  ...
  for (int i = 0; i < WORKSPACE_NUMBER; i++)
    for (struct client *tmp = c_list[i]; tmp != NULL; tmp = tmp->next)
      XChangeProperty(display, root, net_atom[NetClientList], XA_WINDOW, 32,
                      PropModeAppend, (unsigned char *)&(tmp->window), 1);
}
...
static void ewmh_set_desktop_names(void) {
  ...
  Xutf8TextListToTextProperty(display, list, WORKSPACE_NUMBER, XUTF8StringStyle,
                              &text_prop);
  XSetTextProperty(display, root, &text_prop,
...
static int xerror(Display *dpy, XErrorEvent *e) {
  ...
  LOGP("Fatal request. Request code=%d, error code=%d", e->request_code,
       e->error_code);
...
int main(int argc, char *argv[]) {
  ...
  if (conf_path[0] == '\0') {
    ...
    if (xdg_home != NULL) {
      snprintf(conf_path, MAXLEN * sizeof(char), "%s/%s", xdg_home,
               BERRY_AUTOSTART);
    } else {
      ...
      snprintf(conf_path, MAXLEN * sizeof(char), "%s/%s/%s", home, ".config",
               BERRY_AUTOSTART);
    }
  }
